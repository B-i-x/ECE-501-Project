/* STAGING (persistent) from normalized raw tables created by import_csv_to_sqlite.sh
   Source raw tables already in SQLite:
...
SELECT
  ...
  CASE WHEN NULLIF(ABSENT_RATE,'s') IS NOT NULL
       THEN CAST(NULLIF(ABSENT_RATE,'s') AS REAL)/100.0 END AS absence_rate,
  DATA_REP_FLAG,
  PARTIAL_DATA_FLAG,
...
/* Subgroup mapping table (persistent; populated externally via CSV import) */
